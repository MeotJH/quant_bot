...
public class TrendFollowBatchImpl implements TrendFollowBatch{
    ...
    public void sendNotification() {
        ...
        trendFollowAll.forEach(trendFollow -> {
            Boolean status = trendFollow.getNotification().getStatus();
            if(status){
                //상태 바뀌었습니다 확인하세요-> SMS 보내기
            }
        });
    }
    ...
    private void setNotifySignal(List<TrendFollowBatchDto> compares){
        compares.forEach( each -> {
            Boolean status = !each.savedIsBuy().equals(each.todayIsBuy());
